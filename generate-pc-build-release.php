...
if (!function_exists('generatePcBuildReleasePdf')) {
    function generatePcBuildReleasePdf(PDO $pdo, int $buildId, ?string $targetPath = null): string
    {
        ...
        <head>
            <meta charset="UTF-8">
            <title>Protokół wydania PC <?= htmlspecialchars($reference !== '' ? $reference : '#' . $buildId, ENT_QUOTES | ENT_SUBSTITUTE, 'UTF-8') ?></title>
            <style>
                ...
                .signature { margin-top: 24px; min-height: 120px; border: 1px solid #d1d5db; padding: 12px; }
                .signature img { max-height: 100px; }
                .notes { margin-top: 16px; white-space: pre-wrap; }
            </style>
        </head>
        <body>
            <h1>Protokół wydania zestawu PC <?= htmlspecialchars($reference !== '' ? $reference : '#' . $buildId, ENT_QUOTES | ENT_SUBSTITUTE, 'UTF-8') ?></h1>
            <div class="meta">
                ...
                <p><strong>Data wydania:</strong> <?= htmlspecialchars($releaseDate, ENT_QUOTES | ENT_SUBSTITUTE, 'UTF-8') ?></p>
                <p><strong>Sposób dostarczenia:</strong> <?= htmlspecialchars($deliveryMethod, ENT_QUOTES | ENT_SUBSTITUTE, 'UTF-8') ?></p>
            ...
            <h2>Najważniejsze komponenty</h2>
            <table>
                <thead>
                <tr>
                    <th>Kategoria</th>
                ...
                <?php if ($components === []): ?>
                    <tr>
                        <td colspan="3">Brak szczegółów planu komponentów.</td>
                    </tr>
                <?php else: ?>
...
if (basename($_SERVER['SCRIPT_FILENAME'] ?? '') === basename(__FILE__)) {
    ...
    try {
        $pdf = generatePcBuildReleasePdf($pdo, (int) $buildId);
    } catch (Throwable $exception) {
        Response::error('Nie udało się wygenerować protokołu wydania.', 500);
    }

    header('Content-Type: application/pdf');
    header('Content-Disposition: inline; filename="pc-build-release-' . (int) $buildId . '.pdf"');
    echo $pdf;
    exit;
}